   ...
   resizeStream(0)
{
    ...
    {
        Intrinsics::getInstance(368, 368, 256, 212);
    }
    ...
    gui = new GUI(Parse::get().arg(argc, argv, "-sc", empty) > -1);
...
void MainController::followPose()
{
	if (!eFusion)
		return;
...
void MainController::handleGuiChanged()
{
	if (gui->fileToFrame->GuiChanged())
	{
		m_bDeviceSwitched = true;
		
	}
	if (gui->frameToFile->GuiChanged())
	{
		if (gui->frameToFile->Get())
		{
			if (!m_pFrameToFile)
	...
	if (pangolin::Pushed(*gui->deviceOnOff))
	{
		if (m_bDeviceSwitched)
		{
			if (gui->fileToFrame->Get())
			{
			...
			m_bDeviceSwitched = false;
		}
		if (m_pDevice)
		{
			if (m_pDevice->isDeviceOn())
			{
				m_pDevice->stopDevice();
			}
			else
...
void MainController::writeGuiStatus()
{
	if (!eFusion)
		return;
...
void MainController::readGuiStatus()
{
	if (!eFusion)
		return;
    ...
    while(!pangolin::ShouldQuit())
    {
		if (!gui->pause->Get())
		{
			...
			if (pCurrentFrame)
			{
				...
				pVisualFrame->acquireFromCompositeFrame(pCurrentFrame.get());
				...
				if (groundTruthOdometry)
				{
					...
					*currentPose = groundTruthOdometry->getTransformation(pVisualFrame->getTimeStamp());
				}
				if (eFusion)
				{
					...
					if (pColorBuffer)
					{
						const int nBufferNum = pVisualFrame->getWidth() * pVisualFrame->getHeight();
						rgbBuffer = new unsigned char[nBufferNum * 3];
						for (int i = 0; i < nBufferNum; i++)
						{
							rgbBuffer[3 * i] = pColorBuffer[i].rgbRed;
